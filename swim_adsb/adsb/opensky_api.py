...
# -*- coding: utf-8 -*-
#
# Official OpenSky Network API client implementation (extended for OAuth2 Client Credentials)
#
...
class OpenSkyStates(object):
    ...
    def __init__(self, states_dict: Dict[str, Any]):
        """
...
class OpenSkyApi(object):
    ...
    Supports both legacy Basic Auth and OAuth2 Client Credentials (recommended / required for newer accounts).
    """
    ...
    def __init__(
        ...
        If you do not provide any credentials, requests will be anonymous which imposes some limitations.
        For new (non-legacy) accounts, prefer OAuth2 client credentials.

        :param str username: an OpenSky username (legacy; optional).
        :param str password: an OpenSky password for the given username (legacy; optional).
        :param str client_id: OAuth2 Client ID (recommended).
        :param str client_secret: OAuth2 Client Secret (recommended).
        :param str token_url: OAuth2 token endpoint. If not provided, the official OpenSky URL is used.
        :param str scope: Optional OAuth2 scope to request.
        :param requests.Session session: Optional requests session to reuse connections.
        """
        # Legacy basic auth tuple (username, password) or empty tuple when missing
    ...
    def _is_authenticated(self) -> bool:
        """True if using either Basic Auth or OAuth2."""
        return (len(self._auth) == 2) or self._has_oauth()
    ...
    def _fetch_access_token(self, force: bool = False) -> Optional[str]:
        ...
        :param bool force: ignore cache and force refresh.
        :return: access token string or None if OAuth is not configured.
        """
        ...
        if r.status_code != 200:
            logger.debug(
                "Token request not OK. Status %d - %s - Body: %s",
                r.status_code,
                r.reason,
                r.text[:500],
            )
        ...
        expires_in = body.get("expires_in", 1800)  # default to 30 minutes if missing
    ...
    # HTTP core
    # -----------------------------
    def _get_json(self, url_post: str, callee: Callable, params: Optional[Dict[str, Any]] = None):
        """
        ...
        elif len(self._auth) == 2:
            # legacy basic auth
            auth = self._auth

        url = f"{self._api_url}{url_post}"
        ...
        if r.status_code == 200:
            ...
            logger.debug(
                "Response not OK. Status %d - %s - Body: %s",
                r.status_code,
                r.reason,
                r.text[:500],
            )
    ...
    def _check_rate_limit(self, time_diff_noauth: int, time_diff_auth: int, func: Callable) -> bool:
        """
    ...
    @staticmethod
    def _check_lat(lat: float):
        if lat < -90 or lat > 90:
            raise ValueError("Invalid latitude {:f}! Must be in [-90, 90].".format(lat))

    @staticmethod
    def _check_lon(lon: float):
        if lon < -180 or lon > 180:
            raise ValueError(
                "Invalid longitude {:f}! Must be in [-180, 180].".format(lon)
            )

    def get_states(self, time_secs: int = 0, icao24: Optional[str] = None, bbox: Tuple[float, float, float, float] = ()):
        """
        ...
        params: Dict[str, Any] = {"time": int(t), "icao24": icao24, "extended": True}
    ...
    def get_my_states(self, time_secs: int = 0, icao24: Optional[str] = None, serials: Optional[Any] = None):
        """
        ...
        """
        if not self._is_authenticated():
            raise Exception("Authentication required for get_my_states (OAuth2 or username/password).")
    ...
    def get_flights_from_interval(self, begin: int, end: int):
        """
    ...
    def get_arrivals_by_airport(self, airport: str, begin: int, end: int):
        """
    ...
    def get_departures_by_airport(self, airport: str, begin: int, end: int):
        """
        Retrieves flights for a certain airport which arrived within a given time interval [begin, end].
    ...
    def get_track_by_aircraft(self, icao24: str, t: int = 0):
        """
